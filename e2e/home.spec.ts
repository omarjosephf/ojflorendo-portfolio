...
test.describe("Homepage", () => {
  test("loads with the OJ Florendo identity and no page/CSP errors", async ({
    page,
  }) => {
    ...
    await expect(page).toHaveTitle(/OJ Florendo/);
  ...
  test("no horizontal overflow on mobile and desktop widths", async ({
    page,
  }) => {
  ...
  test("the project case-study route loads (HTTP 200)", async ({ page }) => {
  ...
  test("axe: no accessibility violations (desktop + mobile)", async ({
    page,
  }) => {
    await page.goto("/");
